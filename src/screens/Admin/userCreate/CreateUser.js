...
function CreateUser() {
  ...
  const handleCreateUser = () => {
    ...
    if (Object.keys(newErrorMessages).length > 0) {
      ...
      const isBasicPlan =
        planDetailsList?.organization?.amount === 1499900 ? true : false;
      const isPlanActive =
        planDetailsList?.organization?.planStatus == 1 ? true : false;
      if (isPlanActive) {
        if (isBasicPlan && getUsersList?.length <= 49) {
          dispatch(
            CreateUserAction(requestData, token, navigation, setIsLoading),
          );
        ...
        setIsLoading(false);
        showAlert(
          'Your subscription plan is inactive.Please activate to create users.',
        );
      }
    }
  };
        ...
        <ScrollView
          ...
          <View>
            ...
            <View style={{margin: 20}}>
              ...
              <ErrorMessage
                errorMessageText={errorMessages.username}
                style={{marginLeft: 5}}
              />
              ...
              <RedButton
                buttonContainerStyle={[
                  ...
                  {marginTop: isPotrait ? '10%' : '5%'},
                ]}
                ButtonContent={isLoading ? <Loader /> : 'CREATE USER'}
